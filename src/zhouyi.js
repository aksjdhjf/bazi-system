...
// 旺衰 → 《易》"时、位"观（潜见惕跃，各当其时）
function shiweiView(strength) {
  if (strength === '身弱')
    return '《易》重"待时"：如《乾》初九"潜龙勿用"，当藏器于身、蓄德俟命，不宜冒进；待风云相济，乃可"见龙在田"。';
  if (strength === '身强')
    return '《易》贵"与时偕行"：如《乾》九三"君子终日乾乾"，自强而不息；然上九"亢龙有悔"，戒在盈满——强极当知敛，勿恃刚而折。';
  return '《易》尚"中和"：如《泰》"天地交而万物通"，刚柔相推、各得其位，宜守常道、顺势而为。';
}
...
function derive(chart) {
  ...
  tuiyan += shiweiView(chart.strength);
  tuiyan += '《系辞》云："穷理尽性，以至于命。"命之理，终须以心性体之、以时势行之，非徒执象数可尽。';

  return { gua, tuiyan, shiwei: shiweiView(chart.strength) };
}
